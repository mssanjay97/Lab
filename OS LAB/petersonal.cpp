...
public:
  CircularBuffer(size_t size1, T v): _size(size1)
	{
		entry = new T[100];
		reset(v);
	}
	...
	{
    if (current + len >= _size) {
    	int first_slice = _size - current - 1;
